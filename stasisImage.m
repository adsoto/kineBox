function imStatc = stasisImage(vid_path,v,preprocess,fr_nums)
...
imMin = double(imCurr).*0+256;
imMax = double(imCurr).*0;
...
imMean = adapthisteq(uint8(std(tmp,0,3)./mean(tmp,3)));
